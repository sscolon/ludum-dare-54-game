    ...
    public class WaveCounterUI : MonoBehaviour
    {
        ...
        private void UpdateUI(int wave)
        {
            ...
            IEnumerator Routine()
            {
                ...
                _rootTransform.sizeDelta = Vector3.zero;
                while(elapsedTime <= 1f)
                {
                    elapsedTime += Time.deltaTime * Speed;
                    float easedTime = Easing.Calculate(elapsedTime, EaseType.In_Out_Cubic);
                    _rootTransform.sizeDelta = Vector3.Lerp(Vector3.zero, Vector3.one, easedTime);
                    yield return null;
                }
                ...
                while (elapsedTime <= 1f)
                {
                    elapsedTime += Time.deltaTime * Speed;
                    float easedTime = Easing.Calculate(elapsedTime, EaseType.In_Out_Cubic);
                    _rootTransform.sizeDelta = Vector3.Lerp(Vector3.one, Vector3.zero, easedTime);
                    yield return null;
                }
